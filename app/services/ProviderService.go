...
`
		...
		for results.Next() {
			err := results.Scan(
				...
				&tenantProvider.EdenAdapter.Enabled,
				&tenantProvider.EdenAdapter.ProviderType.Id,
...
func GetProviderByType(providerType string) ([]models.EdenAdapter, error) {
	databaseString := revel.Config.StringDefault("connectionString", "root:root@tcp(localhost:3306)/eden_tenant?parseTime=true")
	var adapter models.EdenAdapter
	var adapters []models.EdenAdapter
	var err error
